...
# 设置文件路径
csv_path = r'paperwork\2-非定常风速下桥梁有无控制的振动位移3行图片（逐渐递增的风速）\data\Displacement.csv'  # 请替换为实际路径
txt_path = r'paperwork\2-非定常风速下桥梁有无控制的振动位移3行图片（逐渐递增的风速）\data\windspeed.txt'    # 请替换为实际路径

csv_control_path = r'paperwork\2-非定常风速下桥梁有无控制的振动位移3行图片（逐渐递增的风速）\data\datatest100stepsdutyratio.csv'  # 请替换为实际路径

# 读取数据
df = pd.read_csv(csv_path, low_memory=False)
df_control = pd.read_csv(csv_control_path, low_memory=False)

# 创建图形
fig, axs = plt.subplots(3, 1, figsize=(7, 7.5), tight_layout=True)
...
# 3. 流动控制后的位移图
x_2 = df_control['Time'][2:150002].astype(float).values
y_2 = df_control['Laser1234Ave'][2:150002].astype(float).values
sampled_indices = np.arange(0, 150000, 100)
x_2 = x_2[sampled_indices]
y_2 = y_2[sampled_indices]

# 读取RMS & Std
rms_2 = df_control['Laser1234Ave'][0]
std_2 = df_control['Laser1234Ave'][1]

# 绘制流动控制后的位移图
axs[2].plot(x_2, y_2, lw=1)

# 设置位移图格式
axs[2].xaxis.set_minor_locator(ticker.MultipleLocator(25))
axs[2].yaxis.set_minor_locator(ticker.MultipleLocator(1.25))
axs[2].set_ylim(-5.1, 5.1)
axs[2].set_yticks(np.arange(-5, 5.1, 2.5))
axs[2].set_ylabel('Displacement \n(cm)')
axs[2].set_xlabel('Time (s)')

# 设置刻度样式
axs[2].tick_params(length=8, width=1)
axs[2].tick_params(which='minor', length=4, width=1)
axs[2].tick_params(direction='in')
axs[2].tick_params(which='minor', direction='in')

# 添加RMS值
axs[2].text(0.99, 0.90, f'RMS = {rms_2:.3f}', fontsize=22,
            ha='right', va='top', transform=axs[2].transAxes)

# 保存图片
plt.savefig(r'paperwork\2-非定常风速下桥梁有无控制的振动位移3行图片（逐渐递增的风速）\output2.png', dpi=600, bbox_inches='tight')
plt.savefig(r'paperwork\2-非定常风速下桥梁有无控制的振动位移3行图片（逐渐递增的风速）\output2.svg', bbox_inches='tight')
